...
func _draw() -> void:
	...
	if season == "Winter":
		...
		var rain_phase := phase * 5.0
		for index in range(120 if weather != "Storm" else 190):
			# Travel down-left, in the same direction as the painted streak.
			var x: float = fposmod(float(index * 109) - rain_phase * 1.75, game.WORLD_PIXELS.x)
			var y: float = fmod(float(index * 71) + rain_phase * 4.5, game.WORLD_PIXELS.y)
